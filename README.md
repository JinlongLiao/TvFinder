...
- 当前版本：**0.1.0**，个人试用预览版；当前电视已进行部分 USB 实机验证，完整功能仍待验收。
...
1. 安装本机生成的 `交付/电视文件管家-0.1.0-release.apk`。这是独立发布密钥签名的 Release 构建；APK 和私钥不提交 Git。
...
源码构建版支持在应用内播放音视频、逐页查看 PDF、查看文本及排版后的 Markdown、预览 HTML。支持的类型也能选择“用其他应用打开”，未知类型会调用系统应用选择器；最终音视频格式仍取决于电视解码器。USB 直连的应用内播放按需读取，不先复制整文件；交给其他应用打开时会复制到应用临时缓存，过期后清理，原有“导出到电视”仍可使用。HTML 预览允许脚本和外部资源，应用申请网络权限；本地资源仅限预览文件同目录及子目录。不支持 NAS/SMB。已生成的 `交付/电视文件管家-0.1.0-release.apk` 未因本次源码修改自动重建，不能据此认定它包含预览功能。
...
源码采用 Android 字符串资源管理界面和应用错误提示；文件操作按枚举分派，避免语言切换影响业务行为。版本号由 `app/build.gradle` 提供，关于页读取生成的 `BuildConfig`。内置版本说明和许可在构建时从 `docs/`、`LICENSE`、`NOTICE` 同步，避免文档与应用显示不一致。Markdown 排版使用 CommonMark，许可见第三方声明。
...
2026-09-24 在已配对的海信 Android 11 电视和 aigo exFAT U 盘上，已通过独立调试包验证 USB 直连读取目录，以及独立测试目录内创建、写入回读、重命名、复制和删除；测试目录已清理。Release 包已安装并启动，首次运行的存储授权和 Release 包内的 USB 文件操作仍需在电视上完成验收。普通用户文件、4 GiB 以上文件、空间不足、拔插、待机和跨平台构建也仍待验收。
