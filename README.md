...
系统已挂载的文件可直接调用电视已安装的兼容应用。通过 USB 直连的文件需先选择“导出到电视”，存放于电视的 `Download/TV Finder` 后再用兼容应用打开。该页面支持卷内新建目录、复制、移动、重命名、删除，以及通过本应用内部文件浏览页导入单个文件。本版不内置播放器，不支持 NAS/SMB；不申请网络权限。
