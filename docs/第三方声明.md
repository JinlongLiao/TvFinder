...
| `org.commonmark:commonmark` | 0.29.0 | BSD-2-Clause | [Atlassian Pty Ltd 与 commonmark-java 贡献者](https://github.com/commonmark/commonmark-java)；[随包许可](licenses/CommonMark-BSD-2-Clause.txt) |
...
AndroidX Core 与 CommonMark 由项目直接引用，其余主要由它们传递引入；这些是解析到的依赖，不表示每个组件的每个类都实际执行。归属名称指上游项目，逐文件版权以原始源码为准。
